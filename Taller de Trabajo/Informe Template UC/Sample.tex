...
\noindent Al complejo $(C_{\sbullet}(X),\partial_{\sbullet})$ lo llamamos complejo de cadena 
singular. Para ver que el complejo esta bien definido debemos verificar que $\partial^{2}=0$, lo 
que se muestra en el siguiente lema.
...
continua $f:X\to Y$ entre espacios topológicos, obtener un morfismo entre sus grupos de homología, 
para ello la siguiente proposición.
...
\begin{obs}
    ...
    isomorfismo. Por lo tanto, los grupos de homología son un invariante topológico.
\end{obs}

\noindent Lo bueno de esta construcción, a diferencia de otras, es que las 
definiciones no dependen de otra cosa más que el espacio topológico, es decir, de los abiertos 
presentes en la topología. Pero esto viene con un precio a pagar, es prácticamente imposible 
calcular estos grupos usando únicamente la definición. Sin embargo, podemos calcular unos pocos 
ejemplos que resultarán útiles mas adelante.
...
\begin{ej}
    Consideramos los abiertos $A=\S^{1}\setminus\{N\}$ y $B=\S^{1}\setminus\{S\}$. Notemos que 
    $A\cong\R\sim pt$ y del mismo modo $B\sim pt$. Por otro lado, $A\cap B\cong\R\sqcup\R
    \sim pt\sqcup pt$. Por Mayer-Vietoris, para $i>1$ tenemos la secuencia exacta

    \vspace{2mm}
    \centerline{
        \xymatrixcolsep{1.5pc}\xymatrix{
            H_{i}(A\cap B) \ar[r] & H_{i}(A)\oplus H_{i}(B) \ar[r] & H_{i}(\S^{1}) \ar[r] 
            & H_{i-1}(A\cap B)
        }
    }
    \vspace{2mm}
    \noindent Donde $H_{i}(A\cap B)=H_{i}(A)\oplus H_{i}(B)=H_{i-1}(A\cap B)=0$, lo que implica
    que $H_{i}(\S^{1})=0$ para $i>1$, además como es arcoconexo resulta que $H_{0}(\S^{1})=\Z$.
    Queda el caso cuando $i=1$, se tiene la secuencia exacta
    
    \vspace{2mm}
    \centerline{
        \xymatrixcolsep{3pc}\xymatrix{
            H_{1}(A)\oplus H_{1}(B) \ar[r] & H_{1}(\S^{1}) \ar[r]^-{\delta} 
            & H_{0}(A\cap B) \ar[r]^-{i_{A*}\oplus i_{B*}} & H_{0}(A)\oplus H_{0}(B)
        }
    }
    \vspace{2mm}
    \noindent Como $H_{1}(A)\oplus H_{1}(B)=0$, el morfismo $\delta$ es inyectivo y por lo tanto
    $H_{1}(\S^{1})$ es isomorfo a su imagen vía $\delta$, por exactitud, esta resulta ser el
    kernel de $i_{A*}\oplus i_{B*}$.

    \noindent El grupo $H_{0}(A\cap B)$ tiene dos generadores, digamos $p$ y $q$, que corresponden
    a un punto en cada componente de $A\cap B$. Como $A$ es arcoconexo, resulta que 
    $i_{A*}(p)=i_{A*}(q)$, del mismo modo $i_{B*}(p)=i_{B*}(q)$, entonces el morfismo 
    $i_{A*}\oplus i_{B*}$ esta representado matricialmente por
    \begin{equation*}
        \begin{pmatrix}
            \hspace{2mm} 1 & \hspace{2mm} 1 \hspace{2mm} \\
            \hspace{2mm} 1 & \hspace{2mm} 1 \hspace{2mm}
        \end{pmatrix}
    \end{equation*}
    lo que implica que
    \begin{equation*}
        H_{1}(\S^{1})\cong\im{\delta}=\kr{i_{A*}\oplus i_{B*}}\cong\Z
    \end{equation*}
\end{ej}
...
\begin{ej}
    Afirmamos que
    \begin{equation*}
        H_{i}(\S^{n})=\begin{cases}
            \Z &\quad\text{ si} \hspace{2mm} i=0,n \\
            0 &\quad\text{ si} \hspace{2mm} i\neq0,n
        \end{cases}
    \end{equation*}
    Procederemos por inducción sobre $n$, el caso base ya fue demostrado, supongamos que se 
    cumple para $n-1$ con $n>1$. Sea $A=\S^{n}\setminus\{N\}$ y $B=\S^{n}\setminus\{S\}$. Notemos 
    que
    \begin{equation*}
        A\cong\R^{n}\sim pt
        \hhtext{y}
        B\cong\R^{n}\sim pt
    \end{equation*}
    además, $A\cap B\cong\R^{n-1}\times\S^{n-1}\cong\S^{n-1}$. Por Mayer-Vietoris, para $i>2$,
    tenemos la secuencia exacta
        
    \vspace{2mm}
    \centerline{
        \xymatrixcolsep{3pc}\xymatrix{
            H_{i}(A)\oplus H_{i}(B) \ar[r] & H_{i}(\S^{1}) \ar[r]^-{\delta} 
            & H_{i-1}(A\cap B) \ar[r]^-{i_{A*}\oplus i_{B*}} & H_{i-1}(A)\oplus H_{i-1}(B)
        }
    }
    \vspace{2mm}
    \noindent Para $i>1$, el morfismo $\delta$ es isomorfismo y por lo tanto $H_{i}(\S^{1})
    \cong H_{0}(A\cap B)\cong H_{0}(\S^{n-1})$. Por otro lado si $i=1$, el morfismo $\delta$ es
    inyectivo y por un argumento similar al usado en el ejemplo anterior, el morfismo
    $i_{A*}\oplus i_{B*}$ esta representado por
    \begin{equation*}
        \begin{pmatrix}
        ...
        \end{pmatrix}
    \end{equation*}
    entonces $H_{1}(\S^{n})\cong0$ y por arcoconexidad concluimos.
\end{ej}
...
\begin{obs}
    Al igual que en complejos de cadenas, los morfismos $\partial^{i}$ se llaman diferenciales, 
    los elementos en $\im{\partial^{i}}$ se dicen cofronteras y en $\ker{\partial^{i}}$ se dicen 
    cociclos. Además, es claro que $\im{\partial^{i}}\subseteq\kr{\partial^{i+1}}$.
\end{obs}
...
\noindent La demostración de este lema es análoga al caso de complejo de cadena. Previamente,
definimos el complejo de cadenas y los respectivos diferenciales en base a un complejo de cadena, 
usando la idea vista en el ejemplo vamos a definir la cohomología singular.
...
\begin{dfn}
    Sea $R$ un anillo conmutativo y $X$ un espacio. Sean $\phi\in C^{k}(X;R)$ y 
...
Este anillo se conoce como el anillo de cohomología $X$ con coeficientes en $R$.
